...
class MainActivity : AppCompatActivity() {
    ...
    private fun adjustAxes(chart: LineChart) {
        ...
        rightAxis.gridColor = resources.getColor(R.color.gridColor)
        ...
        xAxis.gridColor = resources.getColor(R.color.gridColor)
        xAxis.gridLineWidth = GRID_LINE_WIDTH
        xAxis.textColor = resources.getColor(R.color.xAxisLabelsTextColor)
        xAxis.textSize = 9.0f
     ...
     */
    private fun translateChartIfNeeded(x: Float, newMaxVisibleX: Float,
    ...
    companion object {
         ...
         */
        private val dateFormat: DateFormat = SimpleDateFormat("dd-MM HH:mm:ss",
            Locale.ENGLISH)
         ...
         */
        private class ClientEndpointHandler(mainActivity: MainActivity) :
            Handler(mainActivity.mainLooper) {

            /**
             * Weak reference to Activity passed in the constructor
             */
            private val wrActivity: WeakReference<MainActivity> = WeakReference(mainActivity)

            override fun handleMessage(msg: Message) {
                val mainActivity = wrActivity.get()
                when (msg.what) {
                    ServerMessageTypes.INIT -> {
                        @Suppress("UNCHECKED_CAST")
                        mainActivity?.addDataSet(msg.obj as List<TradeInfo>)
                    }
                    ServerMessageTypes.NORMAL_MESSAGE -> {
                        mainActivity?.addEntry(msg.obj as TradeInfo)
                    }
                }
            }
        }
