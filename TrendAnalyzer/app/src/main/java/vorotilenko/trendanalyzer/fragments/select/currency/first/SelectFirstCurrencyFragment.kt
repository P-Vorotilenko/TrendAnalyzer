...
/**
 * Fragment for selecting the first currency from the list.
 */
class SelectFirstCurrencyFragment : Fragment() {
     ...
     */
    private lateinit var vmFactory: FirstCurrencyVMFactory

    /**
     * Fragment's view model.
     */
    private val viewModel: FirstCurrencyViewModel by viewModels { vmFactory }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        appViewModel.symbolToAdd?.let {
            vmFactory = FirstCurrencyVMFactory(it, appViewModel.observedSymbols)
        }
    }
    ...
    override fun onCreateView(
        ...
        inflater.inflate(R.layout.fragment_select_first_currency, container, false)
    ...
    private fun setRVAdapter(view: View, navController: NavController): CurrenciesAdapter {
        ...
        val adapter = CurrenciesAdapter(appContext, viewModel.shownCurrencies) { currency, _ ->
            appViewModel.symbolToAdd?.apply {
                symbolName = currency.name
                symbolTicker = Currencies.getTicker(currency.name)
            }
            val action = SelectFirstCurrencyFragmentDirections.actionToSelectSecondCurency()
